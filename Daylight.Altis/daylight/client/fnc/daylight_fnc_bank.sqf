...
daylight_fnc_bankWithdraw = {
	_iAmount = round(parseNumber(_this));
...
daylight_fnc_bankDepositOrTransfer = {
	...
	_iAmount = parseNumber(_this select 1);
	...
	if (_iAmount > 0) then {
		...
		if (_vehTarget == player) then {
			...
			if (_iAmountInventory > 0) then {
				...
				// Show message
				[localize "STR_ATM_MESSAGE", format[localize "STR_ATM_MESSAGE_DEPOSIT_SUCCESS", _iAmount, _iAmountInventory - _iAmount, localize "STR_CURRENCY"], true] call daylight_fnc_showHint;
			};
		} else {
			...
			if(_iMoneyBank > _iAmount) then {
				...
				[_vehTarget, player, _iAmount] call daylight_fnc_makeBankTransfer;
...
daylight_fnc_bankTransferReceive = {
	...
	// Show message
	[localize "STR_ATM_MESSAGE", format[localize "STR_ATM_MESSAGE_TRANSFER_RECEIVE_SUCCESS", _iAmountToSave, (_this select 1), localize "STR_CURRENCY", name((_this select 0))], true] call daylight_fnc_showHint;
};