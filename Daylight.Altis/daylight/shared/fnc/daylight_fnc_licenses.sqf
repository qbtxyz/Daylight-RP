...
daylight_fnc_impoundSpawnLicenseSellers = {
...

	{
		...
		_untLicenseSeller setPos (_x select 1);
...
daylight_fnc_licensesAddLicense = {
	...
	// Add license
	_arrLicenses set [count _arrLicenses, (daylight_cfg_arrLicenses select _this) select 0];
...
daylight_fnc_licensesHasLicense = {
	...
	if ((_arrLicenses find ((daylight_cfg_arrLicenses select (_this select 1)) select 0)) != -1) then {
		_bReturn = true
	};
...
daylight_fnc_licensesBuyLicense = {
	...
	_arrSoldLicense = [(daylight_cfg_arrLicenses select (_arrSoldLicense select 0)) select 0, _arrSoldLicense select 1];
	...
	if (([daylight_cfg_iInvMoneyID, _arrSoldLicense select 1] call daylight_fnc_invCheckAmount) >= _arrSoldLicense select 1) then {
		...
		[localize "STR_LICENSE_SELLER_MESSAGE_TITLE", format[localize "STR_LICENSE_SELLER_MESSAGE_PAID", _arrSoldLicense select 1, localize "STR_CURRENCY", _arrSoldLicense select 0, localize "STR_PAIDFROMINVENTORY"], true] call daylight_fnc_showHint;
	} else {
		...
		if (_iMoneyBank >= _arrSoldLicense select 1) then {
			...
			[localize "STR_LICENSE_SELLER_MESSAGE_TITLE", format[localize "STR_LICENSE_SELLER_MESSAGE_PAID", _arrSoldLicense select 1, localize "STR_CURRENCY", _arrSoldLicense select 0, localize "STR_PAIDFROMBANK"], true] call daylight_fnc_showHint;
		} else {
			// No money
	...
	if (_bHasMoney) then {
		_this call daylight_fnc_licensesAddLicense;
...
daylight_fnc_licensesBuyOpenUI = {
	if (!dialog) then {
		...
		for "_i" from 0 to ((count _arrSoldLicense) - 1) do {
			...
			if (!([player, _i] call daylight_fnc_licensesHasLicense)) then {
				lbAdd [1500, format ["%1 (%2%3)", (daylight_cfg_arrLicenses select _i) select 0, (daylight_cfg_arrLicenses select _i) select 1, localize "STR_CURRENCY"]];
